...
// TEMPORARY profiling endpoint (CRON_SECRET-gated). Measures, on the Netlify
// function itself: DB connect/ping, then the full dashboard query workload.
export async function GET(req: Request) {
  ...
  marks.userLookupMs = Math.round(now() - s);
  ...
  if (user) {
    ...
    marks.counts = {
      ...
      upcoming: d.upcomingCheckouts.length,
